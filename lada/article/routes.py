...
def index(tagline=None):
  ...
  if tagline is None:
    articles = Article.query.order_by(desc(Article.posted)).paginate(page, current_app.config['ARTICLE_PER_PAGE'], False)
  else:
    articles = Tag.query.filter_by(line=tagline).first().articles.paginate(page, current_app.config['ARTICLE_PER_PAGE'], False)
